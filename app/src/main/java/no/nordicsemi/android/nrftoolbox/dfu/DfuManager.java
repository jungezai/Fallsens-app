...
public class DfuManager {
	...
	public void connect(Context context, BluetoothDevice device) {
		DebugLogger.d(TAG, "Connecting device");
		mDevice = device;
	...
	private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
			...
			if (newState == BluetoothProfile.STATE_CONNECTED) {
				...
				try {
					...
					if (refresh != null) {
						...
						DebugLogger.d(TAG, "Device cache refresh result: " + success);
					}
		...
		public void onServicesDiscovered(BluetoothGatt gatt, int status) {
			...
			if (status == BluetoothGatt.GATT_SUCCESS) {
				...
				for (BluetoothGattService service : services) {
					DebugLogger.d(TAG, "Found Service: " + service.getUuid());
					if (service.getUuid().equals(DFU_SERVICE_UUID)) {
						DebugLogger.d(TAG, "DFU Service found!");
		...
		public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
			...
			if (status == BluetoothGatt.GATT_SUCCESS) {
				...
				if (characteristic.getUuid().equals(DFU_CONTROLPOINT_CHARACTERISTIC_UUID) && !isFileSizeWritten) {
					DebugLogger.d(TAG, "successfully written startDFU and now writing file size");
					writeFileSize();
					...
					DebugLogger.d(TAG, "successfully written Packet received notification and now writing receive firmware image");
					...
					DebugLogger.d(TAG, "successfully written ReceiveFirmwareImage and now writing file");
					startUploadingFile();
					isReceiveFirmwareImageWritten = true;
				}
			} else {
				DebugLogger.e(TAG, ERROR_WRITE_CHARACTERISTIC + " [" + characteristic.getUuid() + "] Error code: " + status);
		...
		public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
			...
			if (opCode == RECEIVED_OPCODE && request == START_DFU) {
				DebugLogger.d(TAG, "Received notification for StartDFU");
				if (characteristic.getValue()[2] == OPERATION_SUCCESS) {
					DebugLogger.d(TAG, "File length is valid: " + characteristic.getValue()[2]);
					enablePacketNotification();
				} else {
					errorStatus = characteristic.getValue()[2];
					DebugLogger.e(TAG, ERROR_FILE_LENGTH + " (" + errorStatus + ")");
			...
			//here we transfer next packet of 20 bytes
				...
				DebugLogger.d(TAG, "Received Notification for sent Packet");
				...
				DebugLogger.d(TAG, "Bytes received in Packet: " + receivedBytes);
				mCallbacks.onFileTranfering(receivedBytes);
				if (!isLastPacket && !mStopSendingPacket) {
					sendPacket();
				} else {
					DebugLogger.d(TAG, "last packet notification received");
				}
			}
			...
			// if file has been transfered successfully the we will validate transfered file 
				...
				DebugLogger.d(TAG, "File has been transfered");
				if (characteristic.getValue()[2] == OPERATION_SUCCESS) {
					DebugLogger.d(TAG, "Successful File transfer!");
					mCallbacks.onFileTransferCompleted();
					validateFirmware();
				} else {
					errorStatus = characteristic.getValue()[2];
					DebugLogger.d(TAG, ERROR_FILE_TRANSFER + errorStatus);
			...
			//if file has been validated successfully then we will activate and reset device 
				...
				DebugLogger.d(TAG, "Transfered file has been validated");
				if (characteristic.getValue()[2] == OPERATION_SUCCESS) {
					DebugLogger.d(TAG, "Successful File Transfer Validation!");
					mCallbacks.onFileTransferValidation();
					activateAndReset();
					isNotificationEnable = false;
				} else {
					errorStatus = characteristic.getValue()[2];
					DebugLogger.e(TAG, ERROR_FILE_VALIDATION + errorStatus);
	...
	public void disableNotification() {
		if (isNotificationEnable) {
			DebugLogger.d(TAG, "Disable Notification");
	...
	private void writeFileSize() {
		DebugLogger.d(TAG, "writeFileSize");
		if (isDFUServiceFound) {
			...
			DebugLogger.d(TAG, "writing File size");
	...
	private void enablePacketNotification() {
		DebugLogger.d(TAG, "Enable Packet Notification");
	...
	private void receiveFirmwareImage() {
		DebugLogger.d(TAG, "sending Receive Firmware Image message");
	...
	private void startUploadingFile() {
		DebugLogger.d(TAG, "Preparing to send file");
		sendPacket();
	...
	private void validateFirmware() {
		...
		DebugLogger.d(TAG, "writing validate Firmware value");
		mBluetoothGatt.writeCharacteristic(mDFUControlPointCharacteristic);
	}
	...
	private void activateAndReset() {
		...
		DebugLogger.d(TAG, "writing activate and reset value");
		mBluetoothGatt.writeCharacteristic(mDFUControlPointCharacteristic);
	}
	...
	public void openFile(InputStream stream) {
		try {
			...
			DebugLogger.d(TAG, "File Size: " + mFileSize);
		} catch (IOException e) {
			DebugLogger.e(TAG, ERROR_FILE_OPEN + " " + e);
	...
	public void closeFile() {
		if (mFileStream != null) {
			try {
				...
				DebugLogger.e(TAG, ERROR_FILE_CLOSE + " " + e.toString());
	...
	private byte[] getNextPacket() {
		try {
			...
			DebugLogger.e(TAG, ERROR_FILE_READ);
	...
	private void sendPacket() {
		...
		if (mPacketNumber == mTotalPackets) {
			DebugLogger.d(TAG, "This is last packet, packet number: " + mPacketNumber);
			isLastPacket = true;
